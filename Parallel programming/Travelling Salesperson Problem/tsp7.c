...
//Problem parameters
#define CITIES 10000                                    //Αριθμός πόλεων 
...
#define ITERATIONS 10                                   //Αριθμός επαναλήψεων αλγορίθμου
...
void RestartAnts(){                                         //Επαναρχικοποίηση μυρμηγκιών και αποθήκευση καλύτερης απόστασης...
	...
	for(ant = 0; ant < ANTS; ant++){
	    if(ants[ant].tourLength < best){            //Αποθήκευση καλύτερης διαδρομής
...
double AntProduct(int from, int to){                            //Υπολογισμός αριθμητή πιθανότητας επόμενης διαδρομής...
    ...
    float h = 1.0/ distances[from][to];
    return (double) ( hormone[from][to] * pow3(h)); 
}
