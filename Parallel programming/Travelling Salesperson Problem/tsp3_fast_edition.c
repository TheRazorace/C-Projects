...
double HeinritzHsiaoAlgorithm(){                                       //Συνάρτηση υλοποίησης αλγορίθμου
    ...
    int i, j, min_index, cities_left = N-1, index_to_remove;
    double dist, min_dist, total_dist=0;

    min_dist = DIMS*MAP_SIZE*MAP_SIZE;                                     //Ως αρχική μικρότερη απόσταση τίθεται η μέγιστη απόσταση...
                                                                        //...που μπορεί να έχουν δύο πόλεις σε έναν χάρτη 1000x1000 χωρίς ρίζα
    for(i=0; i<N-1; i++){                                                   //Για όλες τις πόλεις
        ...
        cities_left --;                                                     //Μείωση του άνω ορίου για τον δεύτερο βρόγχο  
        RemoveIndex(index_to_remove, cities_left);                          //Πηγαίνει στο τέλος του πίνακα πόλεων η πόλη που βρέθηκε
...
void RemoveIndex(int city1, int city2){
    ...
    temp = cities_indexes[city1];                          //Εναλλαγή των πόλεων
