...
double HeinritzHsiaoAlgorithm(){                                       //Συνάρτηση υλοποίησης αλγορίθμου
    ...
    int i, j, min_index, min_index2, possibility;
    ...
    min_dist2 = DIMS*MAP_SIZE*MAP_SIZE;                                               //...που μπορεί να έχουν δύο πόλεις σε έναν χάρτη 1000x1000
    ...
    for(i=0; i<N-1; i++){                                                   //Για όλες τις πόλεις
        ...
        if(possibility<= POSSIBILITY_OF_SHORTEST){                          //Αν είναι μικρότερος της πιθανότητας που τέθηκε 
            ...
            travel[i+1] = min_index2;                                       //Προστίθεται η θέση της στο ταξίδι
        }

        min_dist = DIMS*MAP_SIZE*MAP_SIZE;                                           //Αρχικοποιέιται ξανά η μικρότερη απόσταση 
        min_dist2 = DIMS*MAP_SIZE*MAP_SIZE;  
