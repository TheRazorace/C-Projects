...
#define NUM_THREADS 8										//Ορισμός αριθμών threads
...
int main(int argc, char *argv[]) {							//Κύρια συνάρτηση
    ...
    /* int max_threads = omp_get_max_threads();             //Εύρεση μέγιστου αριθμού threads
    printf("%d\n", max_threads); */
	//omp_set_dynamic(0); 
    omp_set_num_threads(NUM_THREADS);						//Ορισμός αριθμών threads
	...
	printf("Νήματα που χρησιμοποιούνται: %d\n", NUM_THREADS);
	printf("Αθροιστικές ελάχιστες αποστάσεις των διανυσμάτων από τα κοντινότερα κέντρα:\n");
...
void Classification(void){										//Αντιστοίχιση δεδομένων με το κοντινότερο κέντρο
	...
	#pragma omp parallel for\
			 ...
			 reduction(+: new_dist_sum)							//Οmp statement για παραλληλοποίηση
...
float EuclidianDistance(float * v, float * c){					//Συνάρτηση υπολογισμού ευκλείδειας απόστασης
	...
	for(i=0; i<Nv; i++){										//Για κάθε διάσταση προστίθεται η το τετράγωνο της διαφοράς...
		temp = (v[i] - c[i]);
		dis += temp*temp;											//...στην συνολική απόσταση με χρήση macro
	}
...
void EstimateCenters(void){										//Υπολογισμός νέων κέντρων
	...
	#pragma omp parallel for\
			...
			shared(vec,center)
																//Οmp statement για παραλληλοποίηση
	for(i=0; i<Nc; i++){
		for(j=0; j<N; j++){
