...
delayed_input(int *array, int current, float delay){
	Uint32 echo;
	int new;
...
switch_status_display(int state){
...
int main(void)
{
	...
	short int prev_state = 0, next_state;
	float delay;
	...
	*(unsigned volatile int*)McBSP1_XCR = 0xA0;
	...
	while(1){
		...
		next_state = user_switches_read();

		if((prev_state - next_state)!= 0){
			switch_status_display(next_state);
		}
		prev_state = next_state;
		delay = get_delay_time(next_state);

		prev_echo = delayed_input(delay_array, current, delay);
